...
def doTheEntries(mean, nsigma):
    ...
    return entries, indivNTail, nMin, nMax, nev
       
if len(meanVal) == 1:
    mean = meanVal[0]
    retEntries, thisNTail, nMin, nMax, theNev = doTheEntries(mean, nsigma)
    entries = retEntries
    nTail = thisNTail
    nev = theNev
    if nTail == 0:
    ...
    for aMean in meanVal:
        thisEntry, nTail, newMin, newMax, theNev  = doTheEntries(aMean, nsigma)
    
        totNev += theNev
        for i in range(len(thisEntry)):
            thisEntry[i] += thisEntry[i]
        for i in range(len(thisEntry)):
            thisEntry[i] = thisEntry[i] / 5
        entries = thisEntry
    totMin += newMin
    totMax += newMax
    nMin = totMin/5
